...
addPlayer: function(e){
	...
	if(p.test(e) && result){
        ...
        if (this.listGames[this.lastName].players) {
            this.listGames[this.lastName].total_players = this.listGames[this.lastName].players.length;
        }
    }
...
start: function(listLog){
	...
	listLog.forEach(function(element, index, array){	
		...
		if(a.flag){
			if(self.gameCount>0 && self.listGames["Partida " + self.gameCount])
                games[self.gameCount] = { games: self.lastName, players: self.listGames["Partida " + self.gameCount].players, total_kills: self.listGames["Partida " + self.gameCount].total_kills, total_game: self.listGames["Partida " + self.gameCount].total_game, total_players: self.listGames["Partida " + self.gameCount].total_players, kills: self.listKills, killsMeans: self.listKillsMeans};
		
			self.listKills = {};
			self.listKillsMeans = {};
			self.listPlayers = {};			
            self.lastName = "Partida " + (self.gameCount+1);
            self.listGames[self.lastName] = { hash: a.hash, total_kills: 0, total_game: self.gameCount + 1, total_players:0, players: [], kills: [], kills_by_means:[]};
            self.gameCount++;
		}
